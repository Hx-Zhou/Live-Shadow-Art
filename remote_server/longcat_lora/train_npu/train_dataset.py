...
def build_resolution_map(aspect_ratio_type: str) -> dict:
    if aspect_ratio_type in MULTI_RESOLUTION_MAP:
        return MULTI_RESOLUTION_MAP[aspect_ratio_type]
    if aspect_ratio_type == "mar_768":
        # The official repository only publishes 256/512/1024 buckets.  The
        # 768-stage feasibility run uses the same aspect ratios and scales the
        # 1024 bucket dimensions to multiples of 16 for VAE/latent alignment.
        return {
            key: [float(max(16, round(value / 16) * 16)) for value in (height * 0.75, width * 0.75)]
            for key, (height, width) in MULTI_RESOLUTION_MAP["mar_1024"].items()
        }
    raise ValueError(f"unsupported aspect_ratio_type: {aspect_ratio_type}")
...
class Text2ImageLoraDataSet(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.aspect_ratio = build_resolution_map(self.aspect_ratio_type)
        ...
        with open(txt_root, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                try:
                    ...
                    if not os.path.isfile(data['img_path']):
                        raise FileNotFoundError(data['img_path'])
                    self.data_resolution_infos.append((height, width))
    ...
    def __getitem__(self, index_tuple):
        ...
        raise RuntimeError(f"failed to load a valid training sample after {MAX_RETRY_NUMS} attempts")
